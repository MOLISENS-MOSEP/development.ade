...
# In case the autostart volume was mounted, execute the autostart script if it hasn't been started already
if [ -d "/opt/autostart" ]; then
  if [[ $(pgrep -f "/bin/bash /opt/autostart/autostart.sh") ]]; then
    echo "--- Autostart-volume detected! But autostart is already running, so skip it! ---"
  else
    echo "--- Autostart-volume detected! Starting script... ---"
    /opt/autostart/autostart.sh
  fi  
fi
